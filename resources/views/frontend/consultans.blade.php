      ...
      <div class="col-md-6">
        ...
        <form action="{{ route('consultans.store') }}" method="POST" enctype="">
          ...
          <div class="row">
            <div class="col-md-6 mb-3">
              ...
              <select class="form-select" id="nationalityCountry_id" name="nationalityCountry_id">
                ...
                @foreach ($countries as $country)
                  <option
                    value="{{ $country->id }}" @selected(old('nationalityCountry_id' ?? '' ) == $country->id)>{{ $country->name }}</option>
                @endforeach
            ...
            <div class="col-md-6 mb-3">
              ...
              <select class="form-select" id="residenceCountry_id" name="residenceCountry_id">
                ...
                @foreach ($countries as $country)
                  <option
                    value="{{ $country->id }}" @selected(old('residenceCountry_id' ?? '' ) == $country->id)>{{ $country->name }}</option>
                @endforeach
                <!-- Agrega más opciones según sea necesario -->
              </select>
          ...
          <div class="row">
            <div class="col-md-6 mb-3">
              ...
              <select class="form-select" id="education_id" name="education_id">
                ...
                @foreach ($educations as $education)
                  <option
                    value="{{ $education->id }}" @selected(old('education_id' ?? '' ) == $education->id)>{{ $education->name }}</option>
                @endforeach
            ...
            <div class="col-md-6 mb-3">
              <label for="sectors" class="form-label">Sectores de Interés</label>
              ...
              <ul class="dropdown-menu dropdown_consultants" aria-labelledby="dropdownMenuButton">
                @foreach ($sectors as $sector)
                  <li class="dropdown-item"><label><input type="checkbox"
                                                          name="sectors[]" class="align-middle me-2 item{{$sector->id}}"
                                                          value="{{ $sector->id }}" @checked(in_array($sector->id, old('sectors', [])))>{{ $sector->name }}
                    </label></li>
                @endforeach
              </ul>
