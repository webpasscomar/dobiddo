          ...
          @foreach ($calls as $call)
            <tr>
              ...
              <td class="text-right align-middle" style="white-space: nowrap;">
                <button data-bs-toggle="modal" data-bs-target="#sectorModal{{ $call->id }}"
                  class="btn btn-secondary btn-sm" title="sectores">
                  <i class="fas fa-graduation-cap"></i>
                </button>
                <a href="{{ route('convocatorias.show', $call) }}" class="btn btn-primary btn-sm"
                  title="Vista formulario">
                  <i class="far fa-eye"></i>
                </a>
                <a href="{{ route('convocatorias.edit', $call) }}" class="btn btn-warning btn-sm" title="Editar">
            ...
            <!-- Modal -->
            <div class="modal fade" id="sectorModal{{ $call->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
              aria-hidden="true">
              <div class="modal-dialog modal-dialog-scrollable h-100">
                <div class="modal-content">
                  <div class="modal-header">
                    ...
                    <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal" aria-label="Close"><i
                        class="fas fa-times"></i></button>
                  </div>
                  <div class="modal-body h-100">
                    <form action="{{ route('convocatorias.updateSectors', $call) }}" method="post"
                      id="form{{ $call->id }}">
                      @csrf
                      @method('PATCH')
                      <select name="sectors[]" id="" class="form-control h-100" multiple>
                        @foreach ($sectors as $sector)
                          <option value="{{ $sector->id }}" @selected(in_array($sector->id, $call->sectors->pluck('id')->toArray()))>
                            {{ $sector->name }}</option>
                        @endforeach
                      </select>
                    </form>
